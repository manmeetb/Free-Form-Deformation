...
def plotElements(fig, ax, ElementsSbpData, ElementsFFDData, NumElements,c1,c2,\
    ...
    for i in range(2):
        ...
        if(i == 0):
            #ax.plot_trisurf(zSolid, xSolid, ySolid, cmap=cm.jet, linewidth=0.2)
            Axes3D.scatter(ax, zSolid,xSolid, ySolid, s=20, c='0.75')
            Axes3D.scatter(ax, zFFD, xFFD, yFFD, s=30, c= 'y')
            #Axes3D.plot_wireframe(ax, zFFD, xFFD, yFFD)
        if(i == 1):
...
def changeShape(ElementsFFDDataInit, ElementsSbpDataInit, ElementsSbpDataFinal):
    #Faring
    ElementsSbpDataFinal.append([])
    ElementsSbpDataFinal.append([])
    count = 0
    FFDPointArray = ElementsFFDDataInit[0]
    for dataTuple in ElementsSbpDataInit[0]:
        t = dataTuple[1][0]
        u = dataTuple[1][1]
        v = dataTuple[1][2]
    
        point = computeXYZfromBernsteinSum(t,u,v,FFDPointArray)
        newDataTuple = ((t,u,v),point)
        ElementsSbpDataFinal[0].append(newDataTuple)
        count = count + 1
        if (count %25 == 0):
            print count
    ...
    FFDPointArray = ElementsFFDDataInit[1]
    for dataTuple in ElementsSbpDataInit[1]:
        t = dataTuple[1][0]
        u = dataTuple[1][1]
        v = dataTuple[1][2]
        ...
        point = computeXYZfromBernsteinSum(t,u,v,FFDPointArray)
        newDataTuple = ((t,u,v),point)
        ElementsSbpDataFinal[1].append(newDataTuple)
        count = count + 1
        if (count %25 == 0):
            print count

    #print ElementsSbpDataFinal

def deformLattice(ElementsFFDData):
    # move the faring points
    FFDPointArray = ElementsFFDData[0]
    for i in range(CONST_nXFDD):
        for j in range(CONST_nYFDD):
            for k in range(CONST_nZFDD):
                if((j!= 4) and (k == 4)):
                   for data in FFDPointArray:
                        if(int(data[0][0])==i and int(data[0][1])==j \
                           and int(data[0][2]) == k):
                            data[1][2] = data[1][2] + 50
 
    # move the wing points
    FFDPointArray = ElementsFFDData[1]
    for i in range(CONST_nXFDD):
        for j in range(CONST_nYFDD):
            for k in range(CONST_nZFDD):
                   if((j!= 4) and (k == 0)):
...
def main():
    ...
    deformLattice(ElementsFFDDataInit)
    changeShape(ElementsFFDDataInit, ElementsSbpDataInit, ElementsSbpDataFinal)
